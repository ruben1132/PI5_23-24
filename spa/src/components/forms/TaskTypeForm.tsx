...
export default function TaskTypeForm(props: Props) {
  // inputs
  const taskTypeName = useFormStringInputWithRegex(
    props.item.value?.name,
    /^[A-Za-z0-9 ]{1,150}$/
  );
  const taskTypeDescription = useFormStringInputWithRegex(
  ...
  const buildingDeleter = useDeleteData(
    config.mgiAPI.baseUrl +
      config.mgiAPI.routes.tasktypes +
      props.item?.value.id
  );
  ...
  const handleSubmitData = async () => {
    ...
    // show alert
    notify.success(
      `Task Type ${props.action == "edit" ? "edited" : "added"} successfully`
    );
  };
            ...
            {props.action === "edit" ? (
              <>
                <Button
                  ...
                  disabled={
                    !taskTypeName.isValid ||
                    !taskTypeDescription.isValid ||
                    !enabled
                  }
                >
                  Update
