...
export default class RoomController implements IRoomController /* TODO: extends ../core/infra/BaseController */ {
    ...
    public async getRooms(req: Request, res: Response, next: NextFunction) {
        try {
            const roomsOrError = (await this.roomServiceInstance.getRooms()) as Result<Array<IRoomWithFloorDTO>>;
    ...
    public async getRoomById(req: Request, res: Response, next: NextFunction) {
        try {
            const roomOrError = (await this.roomServiceInstance.getRoomById(req.params.id)) as Result<
                IRoomWithFloorDTO
            >;
            ...
            return res.json(roomOrError.getValue()).status(201);
        } catch (e) {
            return next(e);
        }
    }

    public async getRoomByName(req: Request, res: Response, next: NextFunction) {
        try {
            const roomOrError = (await this.roomServiceInstance.getRoomByName(req.params.room)) as Result<
                IRoomWithFloorDTO
            >;
