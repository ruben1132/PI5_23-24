...
export default class RobotController implements IRobotController /* TODO: extends ../core/infra/BaseController */ {
    ...
    public async createRobot(req: Request, res: Response, next: NextFunction) {
        try {
            const robotOrError = (await this.robotServiceInstance.createRobot(req.body as IRobotDTO)) as Result<
                IRobotDTO
            >;
    ...
    public async getRobots(req: Request, res: Response, next: NextFunction) {
        try {
            const { taskTypeId, identification } = req.query as { taskTypeId?: string; identification?: string };

            const robotsOrError = (await this.robotServiceInstance.getRobots(
                taskTypeId ?? '',
                identification ?? '',
            )) as Result<Array<IRobotWithRobotTypeDTO>>;
    ...
    public async getRobotById(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const robotOrError = (await this.robotServiceInstance.getRobotById(robotId)) as Result<
                IRobotWithRobotTypeDTO
            >;
    ...
    public async updateRobot(req: Request, res: Response, next: NextFunction) {
        try {
            const robotOrError = (await this.robotServiceInstance.updateRobot(req.body as IRobotDTO)) as Result<
                IRobotDTO
            >;
